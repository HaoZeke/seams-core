...
| ~cages --signature SPEC~ | closed polyhedra whose ring-size census matches ~SPEC~ | ~nop N graph KIND signature S cages C atoms A~ and, with ~--guest-types~, ~guests G occupied O multiple M free F~ |
...
| ~--water-types T,U~ | omitted | ~Neighbour graph on these LAMMPS types only (OW). Substrate and ions never enter the 4-NN list. Distinct from --type (the load filter)~ |
...
| ~--signature SPEC~ | | ~cages: ring-size census (4:6,6:8) or a named table entry (sodalite\vert{}alpha\vert{}512\vert{}51262\vert{}hc\vert{}ddc)~ |
...
~--type~ is the load filter: it drops every other LAMMPS type before
the neighbour graph is built. That is not a water mask. ~--water-types
T,U~ keeps the mixed dump (~loadType = -1~) and builds
~kNearestNeighbourList(cloud, k, cut, types)~ on those types only, so
Ag, I, protein and ions never sit in the 4-NN of OW. ~chill~,
~chill-plus~, ~cages~ (including ~--graph seeded~) and ~ions~ honour
it. Ice scores then count only those types; substrate stays
~unclassified~.

#+begin_src bash
seams chill-plus FILE --water-types 1
seams cages FILE --water-types 1 --graph seeded
seams ions FILE --water-types 1 --ion-types 2,3
#+end_src
