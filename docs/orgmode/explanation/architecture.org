...
OpenMP target offload is a second GPU path, meson off by default
(~-Dwith_openmp_offload~, ~SEAMS_OFFLOAD~). It runs Steinhardt and
the TUM ice score (hop-bound six-rings and HC/DDC affiliation).
CHILL+ stays on the host. It is not gpulite.
