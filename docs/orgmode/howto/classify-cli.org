...
[[file:ionic-liquid.org][Ionic-liquid dumps]]. ~--type~ is the load
filter, not the neighbour mask. AgI, brine, protein and kaolinite
dumps keep every atom and pass ~--water-types T~ (OW) so substrate
and ions never enter the 4-NN list:
...
~--family~ defaults to ~waterIce~. ~chill~ / ~chill-plus~ / ~cages~
refuse every other family. A triclinic dump uses the stored tilt; see
