...
six-rings and HC/DDC affiliation) for an NVIDIA GPU. This is not
the gpulite CUDA batch and it is not the host-only pixi
environment. Device CHILL+ is not in this path.
...
release). clang with libomptarget is the fallback (~-fopenmp
...
The Catch2 case ~steinhardtQl offload matches serial and threaded
bit for bit~ compiles only when the probe succeeded. It compares
...
The case ~TUM offload cage counts match host on mW cubic~ compares
~ring::tumIceScore~ under ~SEAMS_OFFLOAD=0~ and ~SEAMS_OFFLOAD=1~
on the same mW cubic frame: six-ring count, HC/DDC ring counts,
and per-atom cage labels. ~seams cages~ uses
~primitive::sixRingNetwork~ so ~SEAMS_OFFLOAD=1~ takes the device
TUM path. CHILL+ stays on the host.
