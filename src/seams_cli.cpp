...
void printCounts(std::ostream &os, const Cloud &cloud,
  ...
  for (const auto &pt : cloud.pts) {
    if (!waterTypes.empty() && !isWaterType(pt.type, waterTypes)) {
      continue;
    }
    hist[iceName(pt.iceType)]++;
    ++nScore;
  }
  os << colorizer.heading("nop") << " "
     << (waterTypes.empty() ? cloud.nop : nScore);
...
int cmdCages(std::ostream &os, Cloud &cloud, double cutoff, int typeI, int k,
  ...
  const auto tallyAtoms = [&](const std::vector<bool> &hc,
    ...
    for (int i = 0; i < n; ++i) {
      if (!waterTypes.empty() && i < cloud.nop &&
          !isWaterType(cloud.pts[static_cast<std::size_t>(i)].type,
                       waterTypes)) {
        continue;
      }
  ...
  os << colorizer.heading("nop") << " " << nScore << " "
     << colorizer.longOption("graph") << " " << graphName << " "
